...
def GammaTrans(img,c=255,gamma=1.5):
...
def GenerateLab(rgb_img):
    ...
    lab_img[:,:,0]=GammaTrans(lab_img[:,:,0],gamma=0.8)
...
def GenerateHSI(rgb_img):
    ...
    hsi_img[:,:,2]=GammaTrans(hsi_img[:,:,2],gamma=1.7)
    return HSIToRGB(hsi_img)

def GenerateRGB(rgb_img,channel=3):
    new_img=np.zeros(rgb_img.shape,dtype=np.float)
    if channel==1:
        new_img=LaplacianSharpen(rgb_img)
        for i in range(0,6):
            new_img=LogTrans(new_img)
        new_img=GammaTrans(new_img,gamma=1.25)
