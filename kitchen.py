...
def GammaTrans(img,c=255,gamma=2.1):
...
def InvLogTrans(img):
    new_img=np.exp(img/255)
    min_v=np.min(new_img)
    new_img=new_img-min_v
    max_v=np.max(new_img)
    new_img=np.clip(255*(new_img/max_v),0,255)
    return new_img

def LogTrans(img):
    new_img=np.log(img/255+1)
    max_v=np.max(new_img)
    new_img=np.clip(255*(new_img/max_v),0,255)
    return new_img
...
def GenerateLab(rgb_img):
    ...
    lab_img[:,:,0]=GammaTrans(lab_img[:,:,0],gamma=0.15)
    return XYZToRGB(LabToXYZ(lab_img))

def GenerateHSI(rgb_img):
    hsi_img=RGBToHSI(rgb_img)
    hsi_img[:,:,2]=LaplacianSharpen(hsi_img[:,:,2])
    hsi_img[:,:,2]=GammaTrans(hsi_img[:,:,2])
    hsi_img[:,:,2]=HistogramEqual(hsi_img[:,:,2])
    hsi_img[:,:,2]=GammaTrans(hsi_img[:,:,2],gamma=0.46)
    return HSIToRGB(hsi_img)

def GenerateRGB(rgb_img,channel=3):
    new_img=np.zeros(rgb_img.shape,dtype=np.float)
    if channel==1:
        up_img=InvLogTrans(rgb_img)
        for i in range(0,12):
            up_img=InvLogTrans(up_img)
        for i in range(0,9):
            up_img=GammaTrans(up_img,gamma=0.9)
            up_img=LogTrans(up_img)
        up_img=LaplacianSharpen(up_img,mode=1)
        low_img=GammaTrans(rgb_img,gamma=0.5)
        low_img=LaplacianSharpen(low_img)
        for h in range(0,rgb_img.shape[0]):
            for w in range(0,rgb_img.shape[1]):
                if (0<=h<=254 and 16<=w<=322 and not(131<=h<=284 and w>317)) or (0<=h<=123 and 0<=w<=25) or (12<=w<=64 and h<=275):
                    new_img[h][w]=up_img[h][w]
                else:
                    new_img[h][w]=low_img[h][w]
